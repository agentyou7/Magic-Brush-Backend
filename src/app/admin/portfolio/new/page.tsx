...
const NewPortfolioPage = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const portfolioId = `${portfolioItem.title
        .toLowerCase()
        .trim()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')}-${Date.now()}`;
    ...
    <div className="mx-auto max-w-4xl">
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-900">Add Portfolio Item</h1>
        </div>
      </div>
          ...
          <div className="mx-4 w-full max-w-sm rounded-2xl bg-white p-8 text-center shadow-2xl">
            {saveState === 'saved' ? (
              <>
                ...
                <h3 className="text-xl font-semibold text-slate-900">Portfolio Saved</h3>
                <p className="mt-2 text-sm text-slate-600">
                  Your new portfolio item has been saved successfully.
                </p>
              </>
            ) : (
              <>
                ...
                <h3 className="text-xl font-semibold text-slate-900">Saving Portfolio</h3>
                <p className="mt-2 text-sm text-slate-600">
                  Please wait while we save your portfolio item.
                </p>
              </>
            )}
          </div>
