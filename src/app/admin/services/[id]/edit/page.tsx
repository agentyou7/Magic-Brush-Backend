...
const EditServicePage = () => {
  ...
  useEffect(() => {
    ...
    const loadService = async () => {
      try {
        ...
        const matchedService = Array.isArray(result?.data?.services)
          ? result.data.services.find((item: { id: string }) => item.id === serviceId)
          : null;
        ...
        const normalizedFeatures = Array.isArray(matchedService.features)
          ? matchedService.features.slice(0, 3).map((feature: Feature, index: number) => ({
              id: feature.id || `feature_${index + 1}`,
              iconName: feature.iconName || 'Star',
        ...
        setService({
          ...
          shortHeading: matchedService.shortHeading || '',
          description: matchedService.description || '',
          fullDetails: matchedService.fullDetails || '',
          iconName: matchedService.iconName || 'Home',
          imageUrl: matchedService.imageUrl || '',
          imagePublicId: matchedService.imagePublicId || '',
          imageFile: null,
          features: normalizedFeatures.length >= 2
            ? normalizedFeatures
            : [
                { iconName: 'Star', heading: '', description: '' },
                { iconName: 'ShieldCheck', heading: '', description: '' },
              ],
          isActive: matchedService.isActive ?? true,
        });
        ...
        if (isMounted) {
          setError(loadError instanceof Error ? loadError.message : 'Unable to load service right now.');
        }
      } finally {
        if (isMounted) {
  ...
  const renderIcon = (iconName: string) => {
    ...
    if (IconComponent) {
      return React.createElement(IconComponent as LucideIconComponent, { className: "w-5 h-5 text-slate-600" });
    }
    return React.createElement(LucideIcons.Home as LucideIconComponent, { className: "w-5 h-5 text-slate-600" });
  };
  ...
  const isStep2Valid = () => {
    const valid = service.features.every((feature) => {
    ...
    try {
      ...
      try {
        ...
        if (result.success) {
          ...
          setError(result.error || result.message || 'Failed to update service');
        }
      } catch (fetchError) {
      ...
      setError('Failed to update service: ' + (err as Error).message);
    } finally {
      setLoading(false);
                        ...
                        <div className="grid grid-cols-6 gap-4">
                          {filteredIcons.filter(iconName => iconName !== service.iconName).map((iconName) => {
                            ...
                            return (
                              ...
                              >
                                {IconComponent ? 
                                  React.createElement(IconComponent as LucideIconComponent, { className: "w-5 h-5 text-slate-700" }) :
                                  React.createElement(LucideIcons.Home as LucideIconComponent, { className: "w-5 h-5 text-slate-700" })
                                }
                              </button>
                            );
                          })}
                  ...
                  <div className="border border-slate-200 rounded-xl p-4">
                    <div className="flex items-center gap-4">
                      ...
                      <label
                        ...
                        className={`px-3 py-2 border border-slate-300 text-slate-700 rounded-lg transition-colors ${
          ...
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl transform transition-all text-center">
            {saveState === 'saved' ? (
              <>
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">Service Updated</h3>
                <p className="text-slate-600 text-sm">
                  Your service changes have been saved successfully.
                </p>
              </>
            ) : (
              <>
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">Updating Service</h3>
                <p className="text-slate-600 text-sm">
                  Wait, we are saving your changes.
                </p>
              </>
            )}
          </div>
